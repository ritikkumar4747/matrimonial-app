import { useEffect, useState } from "react";
import API from "../services/api";
import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { getAssetUrl } from "../utils/apiConfig";

export default function Sent() {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchSent();
  }, []);

  const fetchSent = async () => {
    try {
      const res = await API.get("/interest/sent");
      setList(res.data);
      setLoading(false);
    } catch (err) {
      console.error(err);
      setLoading(false);
    }
  };

  const getStatusBadge = (status) => {
    switch (status) {
      case "accepted":
        return <span className="badge badge-success">✓ Accepted</span>;
      case "rejected":
        return <span className="badge bg-red-100 text-red-800">✗ Rejected</span>;
      default:
        return <span className="badge bg-yellow-100 text-yellow-800">⏱ Pending</span>;
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-purple-50 to-pink-50 p-6">
      <motion.div 
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="mb-8"
      >
        <h1 className="section-title">Sent Interests</h1>
        <p className="text-gray-600 text-lg">
          You sent {list.length} {list.length === 1 ? 'interest' : 'interests'}
        </p>
      </motion.div>

      {loading ? (
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-primary-500"></div>
        </div>
      ) : list.length === 0 ? (
        <div className="text-center py-20">
          <div className="text-6xl mb-4">💘</div>
          <h3 className="text-2xl font-bold text-gray-700">No interests sent yet</h3>
          <p className="text-gray-500 mt-2">Start exploring matches and send interests</p>
          <Link to="/matches" className="btn-primary mt-6 inline-block">
            Discover Matches
          </Link>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {list.map((interest, index) => (
            <motion.div
              key={interest._id}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.1 }}
              className="card hover:shadow-xl"
            >
              {/* Profile Image */}
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary-400 to-secondary-500 flex items-center justify-center text-white text-2xl font-bold overflow-hidden">
                  {interest.to.photo ? (
                    <img 
                      src={getAssetUrl(interest.to.photo)} 
                      alt={interest.to.name}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    interest.to.name?.[0]
                  )}
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-gray-800">{interest.to.name}</h3>
                  <p className="text-sm text-gray-500">
                    {interest.to.city} • {interest.to.profession}
                  </p>
                </div>
              </div>

              {/* Status Badge */}
              <div className="mb-4">
                {getStatusBadge(interest.status)}
              </div>

              {/* Actions */}
              <div className="flex gap-3">
                <Link
                  to={`/profile/${interest.to._id}`}
                  className="flex-1 py-2 px-4 bg-gradient-to-r from-primary-500 to-secondary-600 text-white rounded-xl font-semibold text-center hover:shadow-lg transition-all duration-300"
                >
                  View Profile
                </Link>
                {interest.status === "accepted" && (
                  <Link
                    to={`/chat/${interest.to._id}`}
                    className="p-2 bg-green-100 text-green-600 rounded-xl hover:bg-green-200 transition-colors"
                    title="Start Chat"
                  >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                  </Link>
                )}
              </div>
            </motion.div>
          ))}
        </div>
      )}
    </div>
  );
}
